...
const ViewStoryModal = ({ confession, visible, onCancel }) => {
    ...
    >
      {!confession ? (
        ...
        <div className="p-4 border border-gray-200 rounded-lg flex flex-col justify-between bg-white shadow-sm relative">
          ...
          {confession?.storyVideoUrl && isVideoMode ? (
            ...
            confession?.storyAudioUrl && (
              <div className="w-full mt-3">
                <AudioPlayer
                  className="w-full bg-primary custom-audio-player"
                  src={imageUrl + confession?.storyAudioUrl}
