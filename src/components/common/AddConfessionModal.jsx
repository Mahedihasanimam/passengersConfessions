...
export const AddConfessionModal = ({
  ...
  const [updateConfession] = useUpdateConfessionMutation();
  ...
  const handleFormSubmit = async (values) => {
    ...
    try {
      if (existingConfession) {
        ...
        const response = await updateConfession({
          id: existingConfession._id,
          data: formData,
        }).unwrap();
        if (response.success) {
          message.success("Confession updated successfully!");
        }
      } else {
        ...
        <div className="flex items-center justify-between py-6 ">
          <div>
            <h2 className="text-[24px] flex space-x-2 items-center font-bold">
              {existingConfession ? "Edit Confession" : "Add Confession"}
            </h2>
          </div>
        </div>
