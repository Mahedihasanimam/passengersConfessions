...
import { useGetAllStoriesQuery } from "../../../redux/apiSlices/stroeisApiSlices";
    ...
    <div className="container mx-auto px-4 pb-[80px]">
      <div className="max-w-4xl mx-auto">
        <h1 className="lg:text-6xl md:text-4xl text-2xl font-bold text-center mt-8 text-secondary pb-2 ">
          RideShare Drivers <span className="text-red-500">CRAZY</span> Stories
        </h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
        {confessionData?.data?.result?.slice(0, 8)?.map((Confession) => (
          <Link key={Confession.id} to={`/stories/${Confession._id}`}>
            <div className="border rounded-lg p-4 shadow-md hover:shadow-lg bg-[#C7C7C740]">
      ...
      {confessionData?.data?.count !== 0 && (
        <div className="w-1/2 mx-auto mt-20 ">
          <Link className="w-full  mx-auto" to="/RideShareStories">
            <Button
              type="primary"
              style={{
